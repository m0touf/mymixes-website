...
export async function createReview(recipeId: number, userId: number | null, data: CreateReviewDTO) {
    const review = await prisma.review.create({
        data: { recipeId, userId: userId ?? undefined, rating: data.rating, comment: data.comment },
    });
